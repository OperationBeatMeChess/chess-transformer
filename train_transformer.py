...
from chess_transformer.chess_transformer.chess_transformer import ChessTransformer
...
MODEL_PATH = "/home/kage/chess_workspace/largeSwinChessNet+critic2.pt"
MODEL_SAVEPATH = "/home/kage/chess_workspace/largeSwinChessNet+critic2.pt"

NUM_ROUNDS = 50
DATASET_SIZE_TRAIN = 380
DATASET_SIZE_TEST = 25
NUM_THREADS = 40

BATCH_SIZE_TRAIN = 3072
BATCH_SIZE_TEST = 3072

WANDB = True
LOG_EVERY = 200
VALIDATION_EVERY = 10_000
...
def run_validation(model, stats):
    ...
    with torch.no_grad():
        for i, (state, action, result) in enumerate(val_loader):
            ...
            policy_output, value_output, critic_output = model(state.unsqueeze(1))
...
if __name__ == "__main__":
    ...
    scheduler = torch.optim.lr_scheduler.LinearLR(optimizer, 1, 0.25, 100_000)
    scheduler = WarmupLR(scheduler, 0.5, 1_000, 'cos')
    ...
    if WANDB:
        wandb.init(project="chess-transformer", id='x0nbk5my', resume='must')
